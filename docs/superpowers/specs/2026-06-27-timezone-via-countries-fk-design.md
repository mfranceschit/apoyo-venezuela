...
The entire migration runs inside a single transaction. A pre-flight check aborts before any DDL if orphaned `places.country` values exist, so either everything succeeds or nothing changes.

```sql
BEGIN;

-- Pre-flight: abort if any places.country has no matching countries.code.
...
If the pre-flight raises, the error message lists the exact `country` values that need to be inserted into `countries` before retrying.
